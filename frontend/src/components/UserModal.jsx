...
const UserModal = ({ user, isEdit, onClose, onSave, allUsers = [] }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // 🔐 Manager auto-assign for manager login
    if (isManager) {
      cleaned.manager = loggedInUser._id;
    }

    // 🔥 FIX: marketing added here
    if (
      ["intern", "employee", "marketing"].includes(cleaned.role) &&
      !cleaned.manager
    ) {
      toast.error("Manager assignment missing");
      return;
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {/* ================= MANAGER SELECT ================= */}
          {(isAdmin || isHR) &&
            ["intern", "employee", "marketing"].includes(formData.role) && (
              <select
                name="manager"
                value={formData.manager}
                onChange={handleChange}
          ...
          {formData.role === "marketing" && (
            <input
              type="text"
              name="teamName"
              placeholder="Marketing Team Name"
              value={formData.teamName}
              onChange={handleChange}
              className="w-full border rounded p-2"
            />
